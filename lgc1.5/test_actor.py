...
action_matrix = abs(female_data)/abs(mix_data)
...
# action_pred = np.zeros(257)
# wav_pred = np.zeros(257)
# for i in range(mix_data.shape[0]):
#     action_one = actor.predict(train_data[i].reshape(1,3,257))
#     action_pred = np.vstack((action_pred, action_one))
#
# print('action_pred',action_pred.shape)
# w = action_pred[1:,:]*mix_data
action_pred = actor.predict(train_data)
w = action_pred*mix_data
w1 = w[0:10]
w1 = w1.T
print('w',w1.shape)

# wavout_signal = istft(w1, 256, 512, window='hamming',center=False)
# t = wavout_signal.shape[0]
# wav_truth_signal = (female_original[0:t]).tolist()
# wav_truth_noise = (male_original[0:t]).tolist()
# wav_pred_signal = (wavout_signal).tolist()
# wav_pred_noise = (mix[0:t] - wavout_signal).tolist()
# wav_mix = (mix[0:t]).tolist()
#
# wav_truth_signal = matlab.double(wav_truth_signal)
# wav_truth_noise = matlab.double(wav_truth_noise)
# wav_pred_signal = matlab.double(wav_pred_signal)
...
#     print(wav_fenli.shape)
#     # wav_fenli = wav_fenli.real
#
#     wav_fenli = abs(wav_fenli)/np.hamming(512)
#
#     wav_truth_signal = (female_original[512*i:512*(i+1)]).tolist()
#     wav_truth_noise = (male_original[512*i:512*(i+1)]).tolist()
#     wav_pred_signal = (wav_fenli).tolist()
#     wav_pred_noise = (mix[512*i:512*(i+1)] - wav_fenli).tolist()
#     wav_mix = (mix[512*i:512*(i+1)]).tolist()
#
#     wav_truth_signal = matlab.double(wav_truth_signal)
#     wav_truth_noise = matlab.double(wav_truth_noise)
#     wav_pred_signal = matlab.double(wav_pred_signal)
